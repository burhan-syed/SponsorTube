...
const Dropdown = ({
  ...
  return (
    <>
        ...
        <DropdownMenu.Portal id="dropdownportal">
          ...
          >
            {(menuHeader.title || menuHeader.description) && (
              ...
              >
                {menuHeader.title && (
                  <h2 className="text-lg font-semibold">{menuHeader.title}</h2>
                )}
      ...
      {isOpen && (
        <div className="fixed left-0 top-0 z-50 h-full w-full bg-th-invertedBackground/50 backdrop-blur-sm transition-opacity animate-in fade-in sm:pointer-events-none sm:invisible"></div>
      )}
    </>
  );
};
